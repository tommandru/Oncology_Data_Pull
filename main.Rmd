...
FROM( 
        ...
        a.PAT_ID,
        c.INPERSONVSTELE, d.DISEASE_GROUP, d.DISEASE_GROUP_B AS DISEASE_GROUP_DETAIL, e.*, i.RACE, i.MYCHART_STATUS, --i.ETHNICITY
...
update_columns_ethnicity <- glue("UPDATE ONCOLOGY_ACCESS
   SET ETHNICITY_GROUPER = NVL(ETHNICITY_GROUPER, 'BLANK')
 WHERE ETHNICITY IS NULL;")

update_columns_race_grouper <- glue("UPDATE ONCOLOGY_ACCESS
   SET RACE_GROUPER = NVL(RACE_GROUPER, 'BLANK')
 WHERE RACE IS NULL")

update_columns_race_detail <- glue("UPDATE ONCOLOGY_ACCESS
   SET RACE_GROUPER_DETAIL = NVL(RACE_GROUPER_DETAIL, 'BLANK')
 WHERE RACE IS NULL")
